...
import type { ApiResponse, Chapter, BibleClient } from '@americanbible/api-bible-sdk';
import { NotFoundError } from '@americanbible/api-bible-sdk';
import { ApiBibleProvider } from '../src/provider.js';
import { useChapter } from '../src/use-chapter.js';
...
const okResponse: ApiResponse<Chapter> = { data: mockChapter, meta: {} };

// The fake client only needs `chapters.get`; the whole object is cast to
// BibleClient so tests exercise the hook wiring without any HTTP.
function makeClient(get: unknown): BibleClient {
  return { chapters: { get } } as unknown as BibleClient;
}
...
describe('useChapter', () => {
  it('transitions loading -> data and forwards params + the abort signal', async () => {
    ...
    const { result } = renderHook(() => useChapter(BIBLE_ID, CHAPTER_ID, params), {
    ...
    expect(get).toHaveBeenCalledWith(BIBLE_ID, CHAPTER_ID, params, expect.any(AbortSignal));
  });

  it('works without params (undefined forwarded)', async () => {
    const get = vi.fn().mockResolvedValue(okResponse);
    const { result } = renderHook(() => useChapter(BIBLE_ID, CHAPTER_ID), {
      wrapper: wrapperFor(makeClient(get)),
    });

    await waitFor(() => expect(result.current.data).toEqual(mockChapter));
    expect(get).toHaveBeenCalledWith(BIBLE_ID, CHAPTER_ID, undefined, expect.any(AbortSignal));
  });
  ...
  it('stays idle and issues no request when either id is empty', () => {
    ...
    const noBible = renderHook(() => useChapter('', CHAPTER_ID), {
      wrapper: wrapperFor(makeClient(get)),
    });
    expect(noBible.result.current.isLoading).toBe(false);

    const noChapter = renderHook(() => useChapter(BIBLE_ID, ''), {
      wrapper: wrapperFor(makeClient(get)),
    });
