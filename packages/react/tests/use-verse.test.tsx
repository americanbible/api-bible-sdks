...
import type { ApiResponse, Verse, BibleClient } from '@americanbible/api-bible-sdk';
import { NotFoundError } from '@americanbible/api-bible-sdk';
import { ApiBibleProvider } from '../src/provider.js';
import { useVerse } from '../src/use-verse.js';
...
const okResponse: ApiResponse<Verse> = { data: mockVerse, meta: {} };

// The fake client only needs `verses.get`; the whole object is cast to
// BibleClient so tests exercise the hook wiring without any HTTP.
function makeClient(get: unknown): BibleClient {
  return { verses: { get } } as unknown as BibleClient;
}
...
describe('useVerse', () => {
  it('transitions loading -> data and forwards params + the abort signal', async () => {
    ...
    const { result } = renderHook(() => useVerse(BIBLE_ID, VERSE_ID, params), {
    ...
    expect(get).toHaveBeenCalledWith(BIBLE_ID, VERSE_ID, params, expect.any(AbortSignal));
  });

  it('works without params (undefined forwarded)', async () => {
    const get = vi.fn().mockResolvedValue(okResponse);
    const { result } = renderHook(() => useVerse(BIBLE_ID, VERSE_ID), {
      wrapper: wrapperFor(makeClient(get)),
    });

    await waitFor(() => expect(result.current.data).toEqual(mockVerse));
    expect(get).toHaveBeenCalledWith(BIBLE_ID, VERSE_ID, undefined, expect.any(AbortSignal));
  });
  ...
  it('stays idle and issues no request when either id is empty', () => {
    ...
    const noBible = renderHook(() => useVerse('', VERSE_ID), {
      wrapper: wrapperFor(makeClient(get)),
    });
    expect(noBible.result.current.isLoading).toBe(false);

    const noVerse = renderHook(() => useVerse(BIBLE_ID, ''), {
      wrapper: wrapperFor(makeClient(get)),
    });
    expect(noVerse.result.current.isLoading).toBe(false);
